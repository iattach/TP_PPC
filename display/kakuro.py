...
def kakuro(problem, solution=None, figsize=10):
    ...
    kargs = {"color": bg_color, "fontsize": fontsize / 2}
        ...
        for cell in row:
            ...
            if cell.is_black():
                ...
                if cell.bottom:
                    ...
                    bottri = np.array([pos, (pos[0], pos[1] + 1), (pos[0] + 1, pos[1])])
                if cell.right:
                    ax.text(
                        pos[0] + 0.75,
                        pos[1] + 0.65,
    ...
    kargs = {"color": "k", "fontsize": fontsize}
    if solution:
    ...
    # Remove ticks, etc.
    ax.set(xticks=np.arange(ncols), yticks=np.arange(nrows))
    ax.set_xlim(0, ncols)
    ax.set_ylim(0, nrows)
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    ...
    ax.grid(b=True, which="major", color=[0.7, 0.7, 0.7])
