...
from .utils import read_image, get_value_or_domain
...
def n_queens(queens, ax=None):
    ...
    Args:
        ...
        ax: If None, a new axes will be created, otherwise will be used
    ...
    for y, x in enumerate(queens):
        ...
        if isinstance(v, list):
            for x_ in range(n):
                if x_ not in v:
                    ax.imshow(
                        rc, extent=[x_ - 0.35, x_ + 0.35, y - 0.35, y + 0.35], alpha=0.6
                    )
        else:
            if y % 2 == v % 2:
                q = bq
            ...
            if v is not x:
                for x_ in range(n):
                    if x_ != v:
                        ax.imshow(
                            rc,
                            extent=[x_ - 0.35, x_ + 0.35, y - 0.35, y + 0.35],
                            alpha=0.3,
                        )

    # Remove ticks, etc.
    ax.set(xticks=np.arange(n), yticks=np.arange(n))
    ax.set_xticklabels([chr(ord("A") + k) for k in ax.get_xticks()])
