...
def sudoku(problem, solution=None, figsize=(8, 8)):
    ...
    kargs = {"color": "k", "fontsize": get_font_size(figsize[0], N)}
    ...
    for l in range(N):
        for c in range(N):
            ...
            if v > 0:
                s = numbers[v]
                ax.text(c + 0.5, N - 0.5 - l, s, va="center", ha="center", **kargs)
            ...
            for c in range(N):
                if problem[l][c] == 0:
                    ...
                    if isinstance(v, list):
                        print_domain_in_square(ax, N, (l, c), v, numbers=numbers)
                    else:
                        s = numbers[v]
                        ax.text(
                            c + 0.5, N - 0.5 - l, s, va="center", ha="center", **kargs
                        )
    ...
    ax.grid(b=True, which="minor", color=[0.7, 0.7, 0.7])
